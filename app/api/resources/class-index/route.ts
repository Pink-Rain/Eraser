...
export async function POST(request: Request) {
  if (!await authorized()) return NextResponse.json({ error: "Accès refusé." }, { status: 403 })
  try {
    const body = (await request.json()) as { action?: unknown; rowNumber?: unknown; draft?: unknown; classId?: unknown; rank?: unknown; keep?: unknown; remove?: unknown; pairs?: unknown; expectedId?: unknown }
    ...
    if (body.action === "delete" && typeof body.rowNumber === "number") await deleteClassSpell(body.rowNumber)
      ...
      result = await saveClassSpell(body.action === "add" ? null : body.rowNumber as number, body.draft as ClassSpellDraft, { expectedId: typeof body.expectedId === "string" ? body.expectedId : undefined })
