...
const definition = worldIndexDefinitions.creatures

async function IndexData() {
  let data: WorldIndexData | null = null
  let error = ""
  try {
    data = await getWorldIndex("creatures")
  } catch (reason) {
    console.error("WORLD_INDEX_LOAD_FAILED", "creatures", reason instanceof Error ? reason.message : "UNKNOWN_ERROR")
    error = `Le classeur « ${definition.sheetName} » n’a pas pu être chargé depuis Google Drive.`
  }
  return <WorldIndexManager indexKey="creatures" initialData={data} initialError={error} nameOpensDetails />
}
