...
const definition = worldIndexDefinitions.religions

async function IndexData() {
  let data: WorldIndexData | null = null
  let error = ""
  try {
    data = await getWorldIndex("religions")
  } catch (reason) {
    console.error("WORLD_INDEX_LOAD_FAILED", "religions", reason instanceof Error ? reason.message : "UNKNOWN_ERROR")
    error = `Le classeur « ${definition.sheetName} » n’a pas pu être chargé depuis Google Drive.`
  }
  return <WorldIndexManager indexKey="religions" initialData={data} initialError={error} />
}
