...
const definition = worldIndexDefinitions.states

async function IndexData() {
  let data: WorldIndexData | null = null
  let error = ""
  try {
    data = await getWorldIndex("states")
  } catch (reason) {
    console.error("WORLD_INDEX_LOAD_FAILED", "states", reason instanceof Error ? reason.message : "UNKNOWN_ERROR")
    error = `Le classeur « ${definition.sheetName} » n’a pas pu être chargé depuis Google Drive.`
  }
  return <WorldIndexManager indexKey="states" initialData={data} initialError={error} />
}
