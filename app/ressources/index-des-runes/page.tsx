...
const definition = worldIndexDefinitions.runes

async function IndexData() {
  let data: WorldIndexData | null = null
  let error = ""
  try {
    data = await getWorldIndex("runes")
  } catch (reason) {
    console.error("WORLD_INDEX_LOAD_FAILED", "runes", reason instanceof Error ? reason.message : "UNKNOWN_ERROR")
    error = `Le classeur « ${definition.sheetName} » n’a pas pu être chargé depuis Google Drive.`
  }
  return <WorldIndexManager indexKey="runes" initialData={data} initialError={error} />
}
