...
const definition = worldIndexDefinitions.peoples

async function IndexData() {
  let data: WorldIndexData | null = null
  let error = ""
  try {
    data = await getWorldIndex("peoples")
  } catch (reason) {
    console.error("WORLD_INDEX_LOAD_FAILED", "peoples", reason instanceof Error ? reason.message : "UNKNOWN_ERROR")
    error = `Le classeur « ${definition.sheetName} » n’a pas pu être chargé depuis Google Drive.`
  }
  return <WorldIndexManager indexKey="peoples" initialData={data} initialError={error} />
}
