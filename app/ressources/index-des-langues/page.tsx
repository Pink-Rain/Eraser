...
const definition = worldIndexDefinitions.languages

async function IndexData() {
  let data: WorldIndexData | null = null
  let error = ""
  try {
    data = await getWorldIndex("languages")
  } catch (reason) {
    console.error("WORLD_INDEX_LOAD_FAILED", "languages", reason instanceof Error ? reason.message : "UNKNOWN_ERROR")
    error = `Le classeur « ${definition.sheetName} » n’a pas pu être chargé depuis Google Drive.`
  }
  return <WorldIndexManager indexKey="languages" initialData={data} initialError={error} />
}
