...
const definition = worldIndexDefinitions.materials

async function IndexData() {
  let data: WorldIndexData | null = null
  let error = ""
  try {
    data = await getWorldIndex("materials")
  } catch (reason) {
    console.error("WORLD_INDEX_LOAD_FAILED", "materials", reason instanceof Error ? reason.message : "UNKNOWN_ERROR")
    error = `Le classeur « ${definition.sheetName} » n’a pas pu être chargé depuis Google Drive.`
  }
  return <WorldIndexManager indexKey="materials" initialData={data} initialError={error} />
}
