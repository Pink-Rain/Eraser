...
import { listAllCampaignsForAdmin, listCampaignsForMj } from "@/lib/google-sheets"
...
async function SandboxShopData({ accountUid, isAdmin }: { accountUid: string; isAdmin: boolean }) {
  ...
  let loadError = ""
  const [loadedItems, campaigns] = await Promise.all([
  ...
  return <ShopGenerator items={items} loadError={loadError} pageLinked="bac-a-sable" destinationPages={campaigns.map((campaign) => ({ id: campaign.id, name: campaign.name }))} />
}
