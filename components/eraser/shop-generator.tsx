...
/** Modifier un magasin : son nom et son token (devanture, avec le vendeur s'il y en a un). */
function RenameShopDialog({ shop, npcs = [], pending, onClose, onConfirm }: { shop: GeneratedShop; npcs?: CampaignNpcRecord[]; pending: boolean; onClose: () => void; onConfirm: (name: string) => void }) {
  const [name, setName] = useState(shop.name)
  const trimmedName = name.trim()
  const vendorId = "npcId" in shop && typeof shop.npcId === "string" ? shop.npcId : ""
  const vendor = npcs.find((npc) => npc.id === vendorId)
  return <Dialog open onOpenChange={(open) => { if (!open && !pending) onClose() }}><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>Modifier le magasin</DialogTitle></DialogHeader><div className="space-y-4"><label className="grid gap-1.5 text-sm font-medium">Nom<Input value={name} onChange={(event) => setName(event.target.value)} maxLength={200} autoFocus /></label><div className="rounded-xl border bg-muted/25 p-2"><TokenButton kind="shop" ownerId={shop.id.replace(/^latest:/, "")} name={trimmedName || shop.name} source={vendor?.portrait || ""} style={{ kind: "shop", shopKey: shop.key }} /><p className="px-2 pt-1 text-[11px] text-muted-foreground">{vendor ? `Devanture avec ${vendor.name}, le vendeur.` : "Sans vendeur : devanture aux couleurs du magasin."}</p></div><div className="flex justify-end gap-2"><Button type="button" variant="ghost" disabled={pending} onClick={onClose}>Annuler</Button><Button type="button" disabled={pending || !trimmedName} onClick={() => onConfirm(trimmedName)}>{pending ? <LoaderCircle className="animate-spin" /> : <Pencil />}Enregistrer</Button></div></div></DialogContent></Dialog>
}
...
/** Les cartes de magasin passent sur deux colonnes à partir de `xl` (1280 px). */
function useTwoShopColumns() {
  const [twoColumns, setTwoColumns] = useState(false)
  useEffect(() => {
    const query = window.matchMedia("(min-width: 1280px)")
    const update = () => setTwoColumns(query.matches)
    update()
...
function ShopCards({ shops, actions, pending, npcs = [], onSave, onSend, onAdd, onLink, onRename, onDelete, onReroll, onRerollShop, onPriceChange }: { shops: GeneratedShop[]; actions: "none" | "sandbox" | "campaign" | "saved" | "locations"; pending: boolean; npcs?: CampaignNpcRecord[]; onSave?: (shop: GeneratedShop) => void; onSend?: (shop: GeneratedShop) => void; onAdd?: (shop: GeneratedShop) => void; onLink?: (shop: GeneratedShop) => void; onRename?: (shop: GeneratedShop) => void; onDelete?: (shop: GeneratedShop) => void; onReroll?: (shop: GeneratedShop, item: GeneratedShopItem, rarity?: ShopRarity) => void; onRerollShop?: (shop: GeneratedShop) => void; onPriceChange?: (shop: GeneratedShop, item: GeneratedShopItem, price: string) => void }) {
  ...
  // Dans le Créateur de session, les magasins sont fermés par défaut.
  const [openShops, setOpenShops] = useState<Set<string>>(() => new Set())
  const twoColumns = useTwoShopColumns()
  ...
  // Deux magasins côte à côte s'ouvrent et se ferment ensemble : la ligne reste alignée.
  function toggleShop(id: string) {
    const index = shops.findIndex((shop) => shop.id === id)
    const partner = twoColumns ? shops[index % 2 ? index - 1 : index + 1] : undefined
    setOpenShops((current) => {
      const next = new Set(current)
      const open = !next.has(id)
      for (const target of [id, partner?.id].filter((value): value is string => Boolean(value))) { if (open) next.add(target); else next.delete(target) }
      return next
    })
  }
  return <div className="grid gap-4 xl:grid-cols-2">{shops.map((shop) => { const definition = shopDefinitions.find((candidate) => candidate.key === shop.key) ?? shopDefinitions[0]; const Icon = definition.Icon; const linkedNpcId = "npcId" in shop && typeof shop.npcId === "string" ? shop.npcId : ""; const vendor = npcs.find((npc) => npc.id === linkedNpcId); const collapsed = actions === "locations" && !openShops.has(shop.id); return <article key={shop.id} data-shop-id={shop.id} className="deferred-section scroll-mt-24 overflow-hidden rounded-2xl border bg-card/90 shadow-sm target:ring-2 target:ring-primary">
    <header className={`grid grid-cols-[4.75rem_minmax(0,1fr)_auto] items-stretch bg-primary/[0.055] sm:grid-cols-[5.5rem_minmax(0,1fr)_auto] ${collapsed ? "" : "border-b"}`}>
      ...
      {actions !== "none" && <div className="grid shrink-0 grid-cols-2 content-start gap-1 border-l border-primary/15 p-2">
        ...
        {onRename && <Button type="button" size="icon-sm" variant="ghost" disabled={pending} onClick={() => onRename(shop)} aria-label={`Nommer ${shop.name}`} title="Modifier (nom, token)"><Pencil /></Button>}
