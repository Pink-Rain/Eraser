...
import { Check, ChevronDown, ImagePlus, Link2, LoaderCircle, Sparkles, Zap } from "lucide-react"
...
export function CreatureSheetDialog({ open, headers, values, html, onClose, onSave }: {
  ...
  // On choisit parmi les « Sorts des créatures » ; un sort de classe déjà inscrit sur
  // une ancienne fiche garde tout de même sa carte.
  const { options: spells, loading: spellsLoading } = useSpellOptions(["creatures", "classes"])
  ...
  const creatureSpells = spells.filter((spell) => spell.source === "creatures")
  const activeSpells = creatureSpells.filter((spell) => spell.category !== "passif")
  const passiveSpells = creatureSpells.filter((spell) => spell.category === "passif")
  ...
  return <Dialog open={open} onOpenChange={(next) => { if (!next) onClose() }}>
    <DialogContent className="max-h-[92svh] overflow-y-auto sm:max-w-5xl">
      ...
      <SpellPicker label="Actifs" icon={<Zap className="size-3.5" />} value={fields["Sorts actifs"]} options={activeSpells} known={spells} loading={spellsLoading} onChange={(value) => set("Sorts actifs", value)} />
      <SpellPicker label="Passifs" icon={<Sparkles className="size-3.5" />} value={fields["Sorts passifs"]} options={passiveSpells} known={spells} loading={spellsLoading} onChange={(value) => set("Sorts passifs", value)} />
