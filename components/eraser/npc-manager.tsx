...
function randomNpc(pageLinked: string): CampaignNpcRecord {
  ...
  return {
    ...blankNpc(pageLinked), name: randomNpcNames[randomBetween(0, randomNpcNames.length - 1)], currentHp: totalHp, totalHp,
...
function NpcForm({ npc, pending, onClose, onSave }: { npc: CampaignNpcRecord; pending: boolean; onClose: () => void; onSave: (npc: CampaignNpcRecord, portrait?: File) => void }) {
  ...
  return <div className="space-y-6"><section className="grid gap-4 lg:grid-cols-[13rem_minmax(0,1fr)]"><div className="space-y-3"><label className="group relative block aspect-[4/5] cursor-pointer overflow-hidden rounded-2xl border bg-muted/40">{portraitPreview || draft.portrait ? <img src={portraitPreview || draft.portrait} alt={`Portrait de ${draft.name || "ce PNJ"}`} className="size-full object-cover" /> : <div className="grid size-full place-items-center"><UserRound className="size-16 text-primary/25" /></div>}<span className="absolute inset-x-3 bottom-3 flex items-center justify-center gap-2 rounded-lg bg-black/65 px-3 py-2 text-xs text-white opacity-0 backdrop-blur transition group-hover:opacity-100"><ImagePlus className="size-4" />Importer</span><input type="file" accept="image/*" className="sr-only" onChange={(event) => choosePortrait(event.target.files?.[0])} /></label><Label className="grid gap-1.5 text-xs font-semibold text-muted-foreground">Avatar (URL)<Input type="url" value={draft.portrait} onChange={(event) => update("portrait", event.target.value)} placeholder="https://…" /></Label></div><div className="grid content-start gap-4 sm:grid-cols-2"><Label className="grid gap-1.5 text-xs font-semibold text-muted-foreground sm:col-span-2">Nom du PNJ<Input required value={draft.name} onChange={(event) => update("name", event.target.value)} /></Label><NumberField label="PV actuels" value={draft.currentHp} onChange={(value) => update("currentHp", value)} /><NumberField label="PV max" value={draft.totalHp} onChange={(value) => update("totalHp", value)} /><Label className="grid gap-1.5 text-xs font-semibold text-muted-foreground sm:col-span-2">Notes <span className="font-normal">Visible pour les joueurs</span><Textarea value={draft.playerNotes} onChange={(event) => update("playerNotes", event.target.value)} className="min-h-28" /></Label><Label className="grid gap-1.5 text-xs font-semibold text-muted-foreground sm:col-span-2">Notes MJ <span className="font-normal">Visible uniquement par le MJ</span><Textarea value={draft.gmNotes} onChange={(event) => update("gmNotes", event.target.value)} className="min-h-28" /></Label></div></section><section className="rounded-2xl border bg-primary/[0.035] p-4"><div className="mb-3 flex items-center gap-2"><Shield className="size-4 text-primary" /><h3 className="font-display text-lg font-semibold">Caractéristiques</h3></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-6">{statFields.map((field) => <NumberField key={field.key} label={field.label} value={draft[field.key]} onChange={(value) => update(field.key, value)} />)}</div></section><section className="rounded-2xl border p-4"><div className="mb-4"><h3 className="font-display text-lg font-semibold">Sac à dos</h3><p className="text-xs text-muted-foreground">L’unique inventaire du PNJ.</p></div>{draft.createdAt ? <CharacterInventory characterId={draft.id} endpoint={`/api/npcs/${encodeURIComponent(draft.id)}/inventory`} mode="npc" /> : <p className="rounded-xl border border-dashed px-4 py-7 text-center text-sm text-muted-foreground">Sauvegarde d’abord le PNJ, puis rouvre sa fiche pour remplir son Sac à dos.</p>}</section><div className="flex justify-end gap-2"><Button type="button" variant="ghost" onClick={onClose}>Annuler</Button><Button type="button" disabled={pending || !draft.name.trim()} onClick={() => onSave({ ...draft, name: draft.name.trim() }, portraitFile)}>{pending ? <LoaderCircle className="animate-spin" /> : <Save />}Sauvegarder</Button></div></div>
}
