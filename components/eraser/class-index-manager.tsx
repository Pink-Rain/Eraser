...
import { clampTableColumnWidth, usePersistentTableLayout, type PersistentTableLayout } from "@/hooks/use-persistent-table-layout"
...
function EditableSpell({ spell, classes, allSpells, similarities, pending, compact = false, heightOverride, onResizeStart, onResizeReset, onSave, onDelete }: { spell: ClassSpell; classes: ClassRecord[]; allSpells: ClassSpell[]; similarities: SpellSimilarity[]; pending: boolean; compact?: boolean; heightOverride?: number; onResizeStart?: (event: ReactPointerEvent<HTMLSpanElement>, rowNumber: number) => void; onResizeReset?: (rowNumber: number) => void; onSave: (spell: ClassSpell, draft: ClassSpellDraft) => void; onDelete: (spell: ClassSpell) => void }) {
  ...
  return <>
    <tr className="border-b align-top last:border-b-0" style={heightOverride ? { height: heightOverride } : undefined}>
      <td className="sticky left-0 z-10 relative h-full overflow-hidden border-r bg-background px-3 py-2 text-xs tabular-nums text-muted-foreground">
        {spell.rowNumber}
        {onResizeStart && onResizeReset && <span role="separator" aria-label={`Redimensionner la ligne ${spell.rowNumber}`} title="Glisser pour ajuster la hauteur, double-clic pour revenir à l’ajustement automatique" onPointerDown={(event) => onResizeStart(event, spell.rowNumber)} onDoubleClick={() => onResizeReset(spell.rowNumber)} className="absolute inset-x-0 bottom-0 z-30 h-2 cursor-row-resize touch-none" />}
      </td>
      <td className="h-full overflow-hidden border-r p-1.5"><textarea aria-label="Nom du sort" value={draft.name} onChange={(event) => field("name", event.target.value)} className={`eraser-autosize-cell w-full resize-none whitespace-pre-wrap break-words rounded-md border border-transparent bg-transparent px-1.5 py-1 text-sm font-semibold outline-none focus:border-input focus:bg-background ${heightOverride ? "h-full overflow-auto" : ""}`} style={{ overflowWrap: "anywhere", wordBreak: "break-word" }} /></td>
      ...
      <td className="h-full overflow-hidden border-r p-1.5"><textarea aria-label="Type" value={draft.type} onChange={(event) => field("type", event.target.value)} className={`eraser-autosize-cell w-full resize-none whitespace-pre-wrap break-words rounded-md border border-transparent bg-transparent px-1.5 py-1 text-xs outline-none focus:border-input focus:bg-background ${heightOverride ? "h-full overflow-auto" : ""}`} style={{ overflowWrap: "anywhere", wordBreak: "break-word" }} /></td>
      <td className="h-full overflow-hidden border-r p-1.5"><textarea aria-label="Compétences" value={draft.skillsRaw} onChange={(event) => field("skillsRaw", event.target.value)} className={`eraser-autosize-cell w-full resize-none whitespace-pre-wrap break-words rounded-md border border-transparent bg-transparent px-1.5 py-1 text-xs font-semibold text-[#b3261e] outline-none focus:border-input focus:bg-background ${heightOverride ? "h-full overflow-auto" : ""}`} style={{ overflowWrap: "anywhere", wordBreak: "break-word" }} /></td>
      <td className="h-full overflow-hidden border-r p-1.5"><textarea aria-label="Distance" value={draft.distance} onChange={(event) => field("distance", event.target.value)} className={`eraser-autosize-cell w-full resize-none whitespace-pre-wrap break-words rounded-md border border-transparent bg-transparent px-1.5 py-1 text-xs outline-none focus:border-input focus:bg-background ${heightOverride ? "h-full overflow-auto" : ""}`} style={{ overflowWrap: "anywhere", wordBreak: "break-word" }} /></td>
...
export function ClassIndexManager({ initialData, initialError }: { initialData: ResourceData; initialError: string }) {
  ...
  // champ, colonnes "Ligne" et "Actions" figées sur les côtés, en-têtes et
  // barre de recherche fixés en haut, barre de défilement horizontale fixée
  // en bas de l'écran.
  const tableFor = (spells: ClassSpell[]) => <>
    ...
    <div className="sticky bottom-0 z-30 -mx-4 border-t bg-background/95 px-4 backdrop-blur sm:-mx-6 sm:px-6">
      <div ref={bottomScrollRef} onScroll={() => syncScroll("bar")} className="overflow-x-auto overflow-y-hidden" style={{ height: 16 }}>
        <div style={{ width: spellTableWidth, height: 1 }} />
      </div>
    </div>
  </>
  const isTableTab = ["actifs", "passifs", "bonus"].includes(tab)

  return <section className="mt-8">
    <datalist id="class-spell-types">{classSpellTypeSuggestions.map((type) => <option key={type} value={type} />)}</datalist>
    <div className="flex flex-col gap-3 rounded-2xl border bg-card/75 p-4 shadow-sm lg:flex-row lg:items-center lg:justify-end"><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" onClick={() => setTab("duplicates")}><CopyCheck />Voir les doublons</Button>{data.file?.webViewLink && <Button asChild variant="outline"><a href={data.file.webViewLink} target="_blank" rel="noreferrer">Google Sheets <ExternalLink /></a></Button>}<Button type="button" variant="outline" onClick={() => void refresh()} disabled={pending}>{pending ? <LoaderCircle className="animate-spin" /> : <RefreshCw />}Actualiser</Button><Button type="button" onClick={() => startCreate()}><Plus />Créer un sort</Button>{isTableTab && (Object.keys(rowHeightOverrides).length > 0 || Object.keys(tableLayout.columnWidths).some((key) => tableLayout.columnWidths[key] !== defaultTableLayout.columnWidths[key])) && <Button type="button" size="sm" variant="ghost" onClick={() => { setTableLayout(defaultTableLayout); setPreviewWidths({}); setRowHeightOverrides({}) }}><RotateCcw />Réinitialiser la mise en page</Button>}</div></div>
    <div className={`-mx-4 mt-3 border-b bg-background/95 px-4 pb-3 backdrop-blur sm:-mx-6 sm:px-6 ${isTableTab ? "sticky top-14 z-30 pt-3" : ""}`}>
      <div className="relative max-w-xl"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Titre, compétence, type, effet ou description…" className="pl-9" /></div>
    </div>
